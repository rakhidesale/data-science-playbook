#Bydefault it put index on X-axsis
...
plot(airquality$Temp,airquality$Wind,type="p") #p=points Default value is also P
...
plot(airquality$Wind, 
     ...
     main = 'Ozone levels in NY city', #heading
     col = 'green',
     type='l')

plot(airquality,col='red') # plot of entire dataset - all columns(scatter plot - relation between the variables)
...
# Horizontal bar plot
barplot(airquality$Ozone, #col name
        main = 'Ozone Concenteration in air',
        ylab = 'ozone levels', 
        col= 'blue',
        #horiz=T, #True/ by default is vertical
        axes=T)
help(barplot)

#Histogram - frequency distribution and idea about outliers
...
#Single box plot - it directly shows which of the outliers and because of that we are getting skewness in graph/data distribution
help(boxplot) 
...
max(airquality$Solar.R,na.rm=T) # NA remove
